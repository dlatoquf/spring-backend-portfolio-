...
**실제 서비스에서 중요한 데이터 처리와 사용자 경험을 함께 고려하는 개발의 중요성**을 체감했습니다.  

Node.js 기반 개발 경험을 통해 웹 개발에 흥미를 느꼈지만,  
Spring을 활용하면서 **계층 구조 설계와 책임 분리의 중요성**을 깊이 이해하게 되었습니다.  

특히 Controller-Service-Repository 구조와 MyBatis를 활용한 데이터 처리 과정에서  
보다 **체계적인 백엔드 개발 방식에 흥미를 느끼게 되었습니다.**  

또한, 다중 조건 검색 및 페이징 처리 구현을 통해  
**동적 쿼리 기반 데이터 조회 구조 설계 경험**을 쌓을 수 있었으며,  
입력 단계에서 검증 로직을 적용하여  
**데이터 정합성을 유지하는 설계의 중요성**을 실무적으로 이해하게 되었습니다.  

Excel 대량 업로드 기능 구현 과정에서는  
대규모 데이터 처리 시 성능과 안정성을 함께 고려해야 한다는 점을 배우며  
예외 처리와 검증 로직 설계 역량을 강화할 수 있었습니다.  

앞으로는 Spring Boot 및 JPA를 학습하여  
**확장성과 생산성을 고려한 백엔드 개발 역량을 더욱 발전시킬 계획입니다.**
